...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:maria@aig2016@127.0.0.1/aig_probation_form'
...
@app.route("/success")
def success():
    return render_template('senttoemp.html')


@app.route("/document/<string:rev_emp_code>/<string:emp_code>")
def document(rev_emp_code,emp_code):
    the_document = Manager.query.filter(Manager.rev_emp_code == rev_emp_code,Manager.emp_code==emp_code).order_by("id desc").first()

    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    #return str(BASE_DIR)

    return render_template('document.html', the_document=the_document, base_dir=BASE_DIR, emp_code=emp_code)
...
def send_mail(emp_code):
    ...
    emp_code1 = emp_code
    reviewer_email = request.form.get('reviewer_email')
    signature1 = save_signature(request.form.get('signature1'), emp_code1, 'signature1')
    date2 = request.form.get('date2')
